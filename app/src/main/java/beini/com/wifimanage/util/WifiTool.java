...
//        BLog.d("开始扫描 isStart=" + isStart);
        // 得到扫描结果
        mWifiList = mWifiManager.getScanResults();
        BLog.d("  得到扫描结果     mWifiList.size()==" + mWifiList.size());
        // 得到配置好的网络连接
    ...
    public boolean ConnectWifi(int wifiId) {
        for (int i = 0; i < wifiConfigList.size(); i++) {
            ...
            if (wifi.networkId == wifiId) {
                ...
                boolean enable = mWifiManager.enableNetwork(wifiId, true);// 激活该Id，建立连接       返回true不一定代表连接上了
                // status:0--已经连接，1--不可连接，2--可以连接
                BLog.d("激活该Id，建立连接  enable=" + enable + "         String.valueOf(wifi.status)=" + String.valueOf(wifi.status));
//              BLog.d("  mWifiManager.disableNetwork(wifi.networkId)=" + mWifiManager.disableNetwork(wifi.networkId));
                if (!enable) {
                    return false;
                }
                WifiInfo wifiInfo = mWifiManager.getConnectionInfo();
                BLog.d("wifiInfo.getSSID()="+wifiInfo.getSSID()+" wifiInfo.getIpAddress()="+wifiInfo.getIpAddress());
    ...
    public void getConfiguration() {
        ...
        BLog.d("wifiConfigList=" + wifiConfigList.size());
    }
    ...
    public int IsConfiguration(String SSID) {
        for (int i = 0; i < wifiConfigList.size(); i++) {
//            BLog.d("   " + String.valueOf(wifiConfigList.get(i).networkId));
