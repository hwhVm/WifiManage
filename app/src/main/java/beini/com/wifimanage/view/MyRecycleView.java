    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                int dx = xMove - xDown;
                int dy = yMove - yDown;
//                Log.e("com.beini", "Math.abs(dy)=" + Math.abs(dy) + " Math.abs(dx) = " + Math.abs(dx));
                if (Math.abs(dy) < mTouchSlop * 2 && Math.abs(dx) > mTouchSlop) {
                    int scrollX = itemLayout.getScrollX();
                    int newScrollX = mStartX - x;
                    BLog.e("---------->scrollX=" + scrollX + "    newScrollX=" + newScrollX);
                    if (newScrollX < 0) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (Math.abs(dy) < mTouchSlop && Math.abs(dx) < mTouchSlop) {
                    ...
                    int scrollX = itemLayout.getScrollX();
                    if (scrollX > maxLength * 2 / 3) {
                        ((RecycleAdapter) getAdapter()).removeRecycle(pos);
                    } else {
